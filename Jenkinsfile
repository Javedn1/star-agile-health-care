  ...
  stages {
    stage('Git Checkout') {
      steps {
        ...
        git branch: 'master', url: 'https://github.com/Javedn1/star-agile-health-care.git'
                        }
            }
    ...
    stage('Generate Test Report') {
     ...
     stage('Create Docker Image') {
      steps {
        ...
        sh 'docker build -t nabeelj97/healthcare:1.0 .'
                          }
            }
     stage('Login to Dockerhub') {
      steps {
        ...
        sh 'docker login -u ${nabeelj97} -p ${Javedn97@}'
            }
         }
     }
    stage('Docker Push-Image') {
      steps {
        ...
        sh 'docker push nabeelj97/healthcare:1.0'
            }
      }
    stage('AWS-Login') {
      steps {
